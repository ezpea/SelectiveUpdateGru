...
class TestSelectiveUpdateGru(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.batch_size = 2
        self.seq_length = 4
        self.model = SelectiveUpdateGru(self.input_size, self.hidden_size, self.num_layers)        
    ...
    def test_forward_pass(self):
        ...
        out, h = self.model(x)
        self.assertEqual(out.shape, (self.batch_size, self.seq_length, self.hidden_size))
    ...
    def test_selective_update_2(self):
        ...
        padded_out = torch.cat([torch.zeros(self.batch_size, 1, self.hidden_size), out], dim=1)
        
        # outputs dont change when freeze mask is on
        any_of_non_control_output_changed = torch.any(padded_out[:,:,1:].diff(dim=1), dim=-1)
        self.assertTrue(torch.all(torch.where(out[:,:,0]!=0, ~any_of_non_control_output_changed, True)))
